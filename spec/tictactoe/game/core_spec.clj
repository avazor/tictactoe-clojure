...
(defrecord TestEasyAI []
  PlayerStrategy
  (make-move [% % %] 4))
(def human-vs-easy [{:symbol "X" :player (->TestHuman) :type "HUMAN"}
...
(describe "game core"
  ...
  (it "plays a game and ensures that all functions are run in right order with correct args"\
    ;; TODO - simply into SRP tests like below
  ...
  (context "play-game"
    ...
    (around [it]
      (with-redefs [core/play-turn (stub :play-turn {:return [(board/empty-board 9) nil]})
                    io/print-board (stub :print-board)
                    io/print-message (stub :print-message)
                    db/save-game (stub :save-game)
                    db/update-game-status (stub :update-game-status)]
        ))

    (it "prints the board"
      (let [board (board/empty-board 9)]
        (core/play-game human-vs-easy board "Test Game")
        (should-have-invoked :print-board {:with [board]})))

    ;; TODO - add more tests for play-game
    )
  )
